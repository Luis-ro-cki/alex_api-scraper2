...
export async function scraperSpotify(input) {
  return conReintentos(async () => {
    ...
    if (!trackId) {
      throw new Error('Enlace de Spotify inválido');
    }
    ...
    if (!startRes.data?.success && !startRes.data?.id) {
    ...
    // Polling para esperar la conversión
    let attempts = 0;
    ...
    while (attempts < maxAttempts) {
      ...
      try {
        ...
        // Si está completo
        if (status.status === 'completed' || status.status === 'done' || status.downloadUrl) {
          return {
            titulo: status.title || status.name || 'Spotify Track',
            artista: status.artist || status.artists || 'Desconocido',
            album: status.album || null,
            duracion: status.duration || null,
            miniatura: status.thumbnail || status.cover || status.image || null,
            calidad: '320kbps',
            formato: 'audio/mp3',
            audio_url: status.downloadUrl || status.url || status.link
          };
        }
