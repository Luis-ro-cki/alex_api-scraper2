...
/* ===================================================
   SCRAPER 1: DLSRV (embed.dlsrv.online)
   =================================================== */
...
async function youtubeDownloadDlsrv(url, type = 'video', quality) {
  const videoId = extractVideoId(url);
  if (!videoId) throw new Error('[DLSRV] URL inválida');

  // Obtener info
  const infoRes = await axios.post(`${DLSRV_BASE}/api/info`, 
    { videoId }, 
  ...
  if (type === 'audio') {
    const dlRes = await axios.post(
      ...
      { headers: dlsrvHeaders(videoId), timeout: 30000 }
    );
  ...
  // Video: buscar mejor calidad disponible
  ...
  let q = quality ? String(quality).replace(/p$/i, '') : '1080';
  ...
  if (dlRes.data?.status !== 'tunnel' || !dlRes.data?.url) {
    throw new Error('[DLSRV] Enlace de video no generado');
  }
...
async function youtubeDownloadY2mate(url, type = 'video', quality) {
  ...
  for (let i = 0; i < 3 && initData === false; i++) {
    ...
    if (initData === false) await new Promise(r => setTimeout(r, 1000));
  }
...
/* ===================================================
   SCRAPER 3: SAVETUBE (savetube.vip)
   =================================================== */
...
async function youtubeDownloadSavetube(url, type = 'video', quality) {
  const videoId = extractVideoId(url);
  if (!videoId) throw new Error('[Savetube] URL inválida');

  const isVideo = type === 'video';
  const qual = (quality || (isVideo ? '360' : '128')).replace(/[p|kbps]/gi, '');
...
/* ===================================================
   CARRERA TRIPLE (Promise.any)
   =================================================== */

/**
 * Lanza DLSRV, Y2Mate y Savetube EN PARALELO.
 * Devuelve inmediatamente la primera descarga exitosa.
 */
async function youtubeDownload(url, type = 'video', quality) {
  const tasks = [
    youtubeDownloadDlsrv(url, type, quality),
    youtubeDownloadY2mate(url, type, quality),
    youtubeDownloadSavetube(url, type, quality)
  ];
  ...
  try {
    ...
    if (err instanceof AggregateError && err.errors?.length) {
      const msgs = err.errors
        ...
        .join(' · ');
      throw new Error(`Los 3 scrapers fallaron: ${msgs}`);
    }
    throw err;
  }
}
...
/**
 * Export: Descarga de video YouTube (MP4) - CARRERA TRIPLE
 */
export async function scraperYoutubeMp4(input) {
  return conReintentos(async () => {
    ...
    // Intentar descarga con carrera triple
    const resultado = await youtubeDownload(url, 'video');
...
/**
 * Export: Descarga de audio YouTube (MP3) - CARRERA TRIPLE
 */
export async function scraperYoutubeMp3(input) {
  return conReintentos(async () => {
    ...
    // Intentar descarga con carrera triple
    const resultado = await youtubeDownload(url, 'audio');
